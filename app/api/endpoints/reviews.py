...
def list_reviews_for_provider(
    provider_id: UUIDType,
    page: int = Query(1, ge=1),
    size: int = Query(20, ge=1, le=100),
    db: Session = Depends(get_db)
):
    skip = (page - 1) * size
    total, avg = crud_review.count_and_average_for_provider(db, provider_id=str(provider_id))
    items = crud_review.get_paginated_for_provider(db, provider_id=str(provider_id), skip=skip, limit=size)
